...
Install poppler-utils and unzip first. The suite reads the exported PDF back with `pdfinfo` and
`pdftotext`, and the exported Word file with `unzip`. It skips nothing when they are absent: it
fails.
