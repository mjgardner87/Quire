...
### Primary
- **Accent** (default Eucalyptus, `#1f5c4d`): The document's own colour. On the sheet it draws the masthead bar, section headings, credentials line, criterion numbers, achievement dashes and skill separators. In the chrome it draws the active tab's underline, every focus ring, the caret, the primary Export PDF button and the current rail row. The user picks it in the Design panel; the tool writes it to `--accent` on the root and nothing else changes.
...
The chrome follows the system colour scheme; the sheet is paper in both. When the system asks for dark, the desk is `#191d21`, the bar `#22272c`, the rail `#1e2327` and the panels `#272c32`; lines are `#353b42` and `#4a525a`; ink is `#e8ebee`, ink 2 `#c8ced3`, muted `#a3abb2`, quiet `#98a1a8` and faint `#5f676f`; flag amber is `#f2b56b` on `#3b2a10` and danger `#f28b82`; the toast and the primary button carry dark text (`#191d21` and `#10141a`). The document's accent is a dark tone made for white paper, so the chrome lifts it to a fixed lightness and keeps its hue: `oklch(from var(--accent) 0.78 min(c, 0.12) h)`. The lifted accent draws the active tab's underline, the focus rings, the current row's icon and wash, and the primary button. Text on the accent wash is ink, not accent, because a lifted accent on a lifted wash does not hold AA: the current rail row, the active palette row and the chosen segment. Chrome that sits on the sheet (the margin controls, the word badges, the guide label, the empty state) keeps the light set, which is declared on `.sheet` itself. Every chrome text holds 4.5:1 on its own surface in both schemes for all six presets, and the browser test measures it.
...
Dashed lines mean "not yet part of the page": the page-break marker, the guide lines, the add-section button in the rail, the adder under a block and the drop indicator's neighbours. The guide line runs behind the ink and its label sits in the left margin, where nothing else lives, so it covers neither a word nor a word badge. The guides sit where the exporter cuts the pages, and the status line carries that count; the label still says "about here" because the sheet on screen may be set in a different face from the PDF. Borders are 1px throughout; a `kbd` key has a 2px bottom border so it reads as a key.
